#!/usr/bin/env python3
"""
Full-GPU batch processor for Tosano supermarket food-label pipeline — V3 (full-label OCR).
...
V3 = V2 engine optimizations + V1 full-label OCR prompts.
    V2 engine improvements retained:
    ...
    V1 prompts restored (full-label OCR instead of JSON extraction):
    - Context size restored to 16384 (food labels can produce ~1500 tokens)
    - max_tokens=2048 (full label transcription)
    - System prompt: generic OCR (read all text)
    - User prompt: "Transcribe the text in this image."
